---
protocol: Ethena
category: Synthetic dollar
tvl_snapshot: "$4.7B (July 2026 DeFiLlama)"
gtm_archetype: Points meta + CEX rails
main_channels: CEX collateral/pairs; loop venues
incentive_style: Seasonal points on composability loops
---

# Ethena: The Points Meta, Perfected

## Founding wedge

Ethena launched USDe in February 2024: a "synthetic dollar" backed by staked ETH hedged with short perpetual futures, whose funding-rate income makes the dollar itself yield-bearing (sUSDe, the "Internet Bond"). The product's native yield **is** the acquisition hook. In a bull market, sUSDe paid double-digit APY with stablecoin UX.

## GTM & growth levers

- **Seasonal points campaigns.** "Shards" ran until USDe hit $1B supply (April 1, 2024, under two months). The ENA airdrop followed the next day, and "Sats" seasons continued through Season 5+. Points rewarded the highest-leverage behaviors: LPing, locking, and referring.
- **Composability loops manufactured demand.** Pendle (yield-splitting points leverage), Morpho and Aave (looping sUSDe collateral: borrow against a deposit, redeposit the proceeds, repeat) let farmers multiply exposure. Each integration compounded TVL far beyond one-wallet-one-deposit.
- **Narrative timing**: positioned exactly at the intersection of real-yield demand and points-farming appetite. The timing was a lever, not luck.

```
Ethena growth loop: native yield, points, CEX rails

1. Native yield (funding-rate basis trade, sUSDe)
        -> 2. Points seasons (Shards, then Sats; rewards loops)
        -> 3. Composability loops (Pendle, Morpho, Aave looping)
        -> 4. CEX distribution (Binance, Bybit collateral + pairs)
        -> 5. USDe supply grows (fastest stablecoin to $1B+)
        -> 6. Deeper hedge capacity, more yield (scale improves the engine)
        -> loops back to 1

        5 also feeds -> Cyclicality hedge: iUSDe, USDtb (BUIDL),
                         Converge RWA chain with Securitize
```

## Distribution channels

**Centralized exchanges are the moat.** Bybit and Binance listed USDe as margin collateral and spot pairs. The venues that host Ethena's hedges also distribute its product. The Binance embedding alone (reward-bearing collateral, Earn products) exposes USDe to a reported 280M+ registered users; USDe is listed as collateral or spot pairs across most major exchanges (a widely repeated ~60% CEX-market figure could not be independently corroborated). Chain expansion follows leverage demand: Hyperliquid, and Solana venues (Jupiter, Kamino).

TradFi wrappers extend the ladder: iUSDe for fixed-income portfolios, USDtb built with BlackRock's BUIDL, and **Converge**: an RWA-native, EVM-compatible L1 co-developed with Securitize, built to embed Ethena's synthetic dollar and T-bill wrapper into a tokenized-assets stack from day one, not a general-purpose chain expansion. That makes it an institutional GTM hedge against funding-rate cyclicality.

## GTM lessons (inference)

- A product with native yield markets itself. Points amplify value; they don't substitute for it.
- Design points around composability. Reward the loops, and integrations become the growth engine.
- Distribution partners who profit from your success (CEXs earning on USDe pairs and collateral) beat paid placement. Sustainability caveat: funding-rate dependence makes yield cyclical; the TradFi product line is the hedge.
